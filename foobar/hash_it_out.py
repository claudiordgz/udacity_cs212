...
the clutches of Professor Booleans numerous but relatively incompetent minions! Time to plan another escape.

Lucky for you nobody is around (do these security minions just sleep all the time?), so you have a chance to examine
your cell. Looking around, you see no signs of surveillance (ha, they must underestimate you already) and the only thing
keeping you contained is an electronic door lock. Should be easy enough.
...
def digest(current, previous):
    modulo = lambda x, y: x % y
    division = lambda x, y: x / y
    reverse_crypto = lambda n, d, p, op: op(((256 * n) + (d ^ p)), 129)
    reverse_crypto_check = lambda n, d, p: reverse_crypto(n, d, p, modulo)
    reverse_crypto_result = lambda n, d, p: reverse_crypto(n, d, p, division)
    def iterate_crypto(d, p):
        for i in itertools.count():
            if reverse_crypto_check(i, d, p) == 0:
                yield reverse_crypto_result(i, d, p)
    return next(iterate_crypto(current, previous))


def answer(data):
    messages = []
    for i in range(len(data)):
        previous_m = 0 if not messages else messages[i-1]
        messages.append(digest(data[i], previous_m))
    return messages
