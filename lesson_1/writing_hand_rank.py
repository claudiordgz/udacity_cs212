...
def test():
    ...
    f = "4S TS 8S 2S 9S".split()   # Flush
    s = "EC TS 9D 8C 7C".split()   # Straight
    tk = "7H 7D 7C 5C 2C".split()  # Three of a Kind
    tp = "ED EC 3D 3H THD".split() # Two Pairs
    p2 = "2D 2C ED 6H 3C".split()  # Pair of X, High Card
    nada = "7C 5C 3C 3C 2D".split()# I got nothing
    ...
    assert hand_rank(f) == (5, [10, 9, 8, 4, 2])
    assert hand_rank(s) == (4, 11)
    assert hand_rank(tk) == (3, 7, [7,7,7,5,2])
    assert hand_rank(tp) == (2, 11, 3, [13,11,11,3,3])
    assert hand_rank(p2) == (1, 2, [2,2,11,6,3])
    assert hand_rank(nada) == (0, 7, 5, 4, 3, 2)
    return 'tests pass'
